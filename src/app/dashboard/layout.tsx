...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const getActiveTab = () => {
    ...
    if (pathname === '/dashboard/voice') return 'voice';
    return 'overview';
  };
  ...
  return (
    <div className="bg-slate-50/50 text-slate-800 flex-1 font-sans selection:bg-primary-600 selection:text-white flex">
      {/* Left Sidebar */}
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-400 whitespace-nowrap">
                            ...
                            <th className="py-3 px-4">Chat Subject</th>
                          ...
                          {filteredChats.slice((chatsPage - 1) * chatsPerPage, chatsPage * chatsPerPage).map(chat => (
                            <tr key={chat.id} className="hover:bg-slate-50/50 transition-colors group whitespace-nowrap text-xs text-slate-600 font-medium">
                              ...
                              <td className="py-4 px-4">
                                <div className="font-bold text-slate-800 uppercase truncate max-w-[200px]">{chat.title || 'Support Chat'}</div>
                              </td>
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-400 whitespace-nowrap">
                            ...
                            <th className="py-3 px-4">Call Subject</th>
                        ...
                        <tbody className="divide-y divide-slate-100/60">
                          {filteredVoiceLogs.slice((voicePage - 1) * voicePerPage, voicePage * voicePerPage).map(log => (
                            <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group whitespace-nowrap text-xs text-slate-600 font-medium">
                              ...
                              <td className="py-4 px-4">
                                <div className="font-bold text-slate-800 uppercase truncate max-w-[200px]">{log.title || 'Voice Assistant'}</div>
                              </td>
                              <td className="py-4 px-4 text-slate-500">
                                {log.createdAt}
                              </td>
                  ...
                  <div className="md:col-span-5 flex flex-col justify-start overflow-y-auto pr-2 pb-6 min-h-0 scrollbar-thin">
                    ...
                    <div className="py-5 border-b border-slate-100/80 space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-[14px] font-bold text-primary-600">Inquiry Description</h4>
                      </div>
                      <div className="pl-7">
                            ...
                            <tbody>
                              ...
                              {(selectedTicket as any).priority && (
                                <tr>
                                  ...
                                  <td className="py-3 px-4 text-right">
                                    <span
                                      className={`text-[12px] font-extrabold rounded-xl px-3 py-1 border inline-flex items-center gap-1.5 ${(selectedTicket as any).priority === 'HIGH'
                                        ? 'bg-rose-50 border-rose-200 text-rose-700'
                                        : (selectedTicket as any).priority === 'LOW'
                                          ? 'bg-slate-50 border-slate-200 text-slate-600'
                                          : 'bg-amber-50 border-amber-200 text-amber-700'
                                        }`}
                                    >
                                      {(selectedTicket as any).priority === 'HIGH' ? '🔴 High' : (selectedTicket as any).priority === 'LOW' ? '🔵 Low' : '🟡 Medium'}
                                    </span>
                                  </td>
                                </tr>
                              )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">
                      Session ID: {selectedVoiceLog.id}
                    </p>
                  </div>
                </div>
                <button
